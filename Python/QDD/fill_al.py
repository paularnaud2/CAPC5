...
def fill_array_list():
	...
	while gl.counters["col"] < gl.counters["file"]:
		...
		tmp_file_dir = gl.TMP_DIR + '_' + str(gl.counters["col"]) + gl.FILE_TYPE
...
def write_tmp_file_in_array(tmp_file_list):
	...
	counter = 0
	while counter < cur_rm and len(gl.array_list[gl.counters["col"]-1]) < gl.counters["row_max"]:
		counter += 1
		gen_one_line(tmp_file_list[counter-1], gl.array_list[gl.counters["col"]-1])
	return counter
...
def rewrite_tmp_file(tmp_file_list, tmp_file_dir, n_written_rows):
	...
	if len(tmp_file_list) > 0:
		...
		log("Suppression du fichier temporaire No. {}".format(gl.counters["col"]))