...
if not squeeze_export:
    com.log('Export SGE------------------------------------------------------------')
    run_reqList(
        ENV='PROD', BDD='SGE', QUERY_FILE='ReqList/queries/SGE_SUIVI_FIN_TRV_AFF.sql', IN_FILE=in_file, OUT_FILE=out_file, MAX_BDD_CNX=max_bdd_cnx, SL_STEP_QUERY=50, NB_MAX_ELT_IN_STATEMENT=max_elt_st, OPEN_OUT_FILE=False, SQUEEZE_JOIN=True, SQUEEZE_SQL=False, CHECK_DUP=False, SEND_NOTIF=False
    )

if not squeeze_create_table:
    com.log(
        f'Création de la table {table_name}------------------------------------')
    execute(
        ENV='DIRECT', BDD='CAPC5', SCRIPT_FILE='SQL/procs/create_table_aff.sql', VAR_DICT={'@@TABLE_NAME@@': table_name}, PROC=True
    )

com.print_com('')
com.log('Export des données importées dans la table créée----------------------')
sql_import(
    ENV='DIRECT', BDD='CAPC5', SCRIPT_FILE='SQL/scripts/insert_table_aff.sql', VAR_DICT={'@@TABLE_NAME@@': table_name}, IN_DIR=out_file, NB_MAX_ELT_INSERT=max_elt_insert
)

com.print_com('')
com.log(
    f'Mise à jour de la vue {view_name}-----------------------------------')
execute(
    ENV='DIRECT', BDD='CAPC5', SCRIPT_FILE='SQL/scripts/update_view_aff.sql', VAR_DICT={
