...
def remove_dup_main(in_dir = IN_FILE, out_dup_dir = OUT_DUP_FILE, out_dir = OUT_FILE, field_nb = 0):
	
	log("Suppression des doublons dans le fichier {} en cours...".format(in_dir))
	if field_nb == 0:
		cur_list = load_txt(in_dir)
	else:
		cur_list = load_csv(in_dir)
		cur_list = [elt[field_nb - 1] for elt in cur_list if elt[field_nb - 1] != '']
	bn = big_number(len(cur_list))
	log("Fichier chargé, {} lignes à analyser.".format(bn))
