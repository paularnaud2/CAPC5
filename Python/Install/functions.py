...
def init(install = True):
	...
	if str(sys.version).find("32 bit") != -1:
		gl.package_list = load_csv(gl.CONF_PATH32)
	else:
		gl.package_list = load_csv(gl.CONF_PATH64)
...
def install():
	...
	for elt in gl.package_list:
		...
		if is_installed(cur_mod_name):
			...
			cmd_path = gl.PACKAGE_PATH + '\\' + cur_mod_file_name
			cmd = [gl.PIP_PATH, 'install', cmd_path]
			print_com("Commande :")
			print_array(cmd, 1)
			if run_cmd(cmd):
